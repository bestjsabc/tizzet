...
module Refinery::ApplicationHelper
  ...
  def browser_title(yield_title=nil)
    ...
    # (@meta.browser_title.nil? ? @meta.browser_title : @meta.path unless @meta.path.nil? || @meta.browser_title.nil?),
  end
  ...
  def image_fu(image, thumbnail = nil , options={})
    if image.present?
      ...
      image_tag image_thumbnail.public_filename, {:alt => "", :width => image_thumbnail.width, :height => image_thumbnail.height}.merge!(options)
    end
  end
  ...
  def jquery_include_tags(use_caching=RefinerySetting.find_or_set(:use_resource_caching, false))
    if !local_request? and RefinerySetting.find_or_set(:use_google_ajax_libraries, true)
      ...
      #{javascript_include_tag 'jquery-ui-1.8.js'}"
    else
      javascript_include_tag 'jquery', 'jquery-ui-1.8.js', :cache => (use_caching ? "cache/libraries" : nil)
    end
  end
